"""
Ваша задача - создать Python-сценарий, который извлекает данные по каждому фильму ( по каждой ссылке, сохраненной в url_joined) и сохраняет их в JSON-файл.
    ...
    for link in soup.find_all(
        ...
        if a_tag:
            release_links.append(
                a_tag.get("href")
            )  # Добавление ссылки на фильм в список

    # Преобразование относительных ссылок в абсолютные
    url_joined = [
        urllib.parse.urljoin("https://www.boxofficemojo.com", link)
